...
import { useFetchDelete } from '../../hooks';
...
function Blog(props) {
    ...
    const handleDelete = useCallback(async()=>{
        setIsdinalog(false)
        await useFetchDelete(`${import.meta.env.VITE_APP_SERVER_URL}/api/blog/${idDelete}`,setStatus)
    ...
    return (
        <div className='center-element'>
            ...
            <div className="w-[98%] shadow-lg mt-[80px] mb-5 p-4 bg-white rounded-md mx-auto flex justify-between">
                <div className="text-[24px] my-auto text-blue-900 font-[600] tracking-[0.75px]">Blog</div>
                <Link to={'/blogs/add'} className='px-4 py-2 bg-[#0F09A1] select-none rounded-md shadow-md hover:scale-105 active:scale-90 cursor-pointer duration-150 text-white' >Add new blog</Link>
            </div>
            {/* End Header */}
            <div className='bg-white rounded-md w-[98%] shadow-lg mb-5 p-4 mx-auto'>
                ...
                <div className='shadow-md'>
                    <DataGridDemo type={'blog'} handler={handleDinalog}/>
                </div>
            </div>
            {/* Footer */}
            <Footer/>
