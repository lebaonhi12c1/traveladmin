...
function New({ type }) {
    ...
    const [tourDetination, setTourDestination] = useState(null);
        ...
        switch (type) {
            case "destination":
                try {
                    const result = await getUpCloudinary(
                        ...
                        image,
                        `/destination/${convertToValidDirectoryName(
                            destination.name
                        )}`
                    );
                    if (result) {
            ...
            case "tour":
                try {
                    ...
                    if (result) {
                        const { content, imageUrls, public_id_cloud } =
                            await processImages(
                                editorRef.current.getContent(),
                                `/tour/${convertToValidDirectoryName(
                                    tour.title
                                )}`
                            );
                        await useFetchCreate(
                            `${env.VITE_APP_SERVER_URL}/api/tour`,
                            {
                                ...tour,
                                destination: tourDetination === '' ? desinationTour[0] : JSON.parse(tourDetination),
                                content: content,
                ...
                try {
                    ...
                    if (result) {
                        const { content, imageUrls, public_id_cloud } =
                            await processImages(
                                editorRef.current.getContent(),
                                `/blog/${convertToValidDirectoryName(
                                    blog.title
                                )}`
                            );
                        await useFetchCreate(
                            `${env.VITE_APP_SERVER_URL}/api/blog`,
                            {
                                ...blog,
            ...
            <div className="mt-[14px] ml-[250px] px-4">
                <div className="grid grid-cols-2 gap-4">
                    <div className="flex flex-col gap-10 p-4 bg-white shadow-lg justify-center items-center">
                        <div className="uppercase font-medium text-[24px]">
                            {type}
                        </div>
                            ...
                            {data.map((item, index) => (
                                <div className="flex gap-2" key={index}>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) =>
                                            item.state.handlers(e.target.value)
                                        }
